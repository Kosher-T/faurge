...
- **`core/watchdog.py` & `systemd/faurge-watchdog.service`**: The VRAM monitoring daemon running as a persistent, auto-restarting systemd service.
...
- **`core/vram_policy.py` & `scripts/vram_budget.py`**: Loads models, runs a short inference loop to profile memory fragmentation, and halts the build if peak VRAM exceeds the safety margin (≥ 3.9GB).
- **`core/memory_tracker.py`**: Detailed peak/per-layer memory profiling to debug budget overruns.
- **`tests/test_vram_budget.py`**: Validates the budget logic using mock models.
...
- **`core/gym_env.py` & `gym_env/test_env.py`**: Ursula's offline DSP simulator and its sanity checks.
- **`scripts/quantize_models.py`**: Post-training float16/INT8 quantization tools to drastically reduce VRAM footprints.
...
- **`training/train_all.py`**: Meta-script orchestrating the sequential training runs.
...
- **`scripts/export_onnx.py` & `scripts/export_tflite.py`**: Inference optimization pipelines.
...
- **`core/audio_processor.py` & `core/state_manager.py`**: Encapsulates real-time processing, buffer math, and the global bypass/active state machine.
...
- **CI/CD (`.github/workflows/ci.yml`)**: Matrix testing, caching, and simulated VRAM-budget checks on every push.
...
| Gate | Condition |
|------|-----------|
| Phase 1 → 2 | `test_dsp_connection.py` cleanly exits; hardware dry bypass is confirmed. |
| Phase 2 → 3 | `vram_budget.py` completes without exceeding the safety margin. |
| Phase 3 → 4 | Objective `qa_metrics.py` pass; checkpoints are registered in MLflow. |
| Live Bake | Immutable bake manifest and rollback snapshot are committed. |