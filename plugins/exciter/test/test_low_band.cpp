...
TEST(low_band_bypass_at_zero_sub_level) {
    auto input = makeSine(200.0f, 0.5f, 48000, 0.05f);
    std::vector<float> output(input.size());

    faurge::LowBand lb;
    lb.process(input.data(), output.data(), input.size(), 48000, 0.0f, 0.0f);
...
TEST(low_band_silence_in_silence_out) {
    std::vector<float> input(4800, 0.0f);
    std::vector<float> output(input.size());

    faurge::LowBand lb;
    lb.process(input.data(), output.data(), input.size(), 48000, 6.0f, 1.0f);
...
TEST(low_band_sub_level_controls_energy) {
    auto input = makeSine(200.0f, 0.5f, 48000, 0.1f);
    std::vector<float> outLow(input.size()), outHigh(input.size());

    faurge::LowBand lb;

    lb.process(input.data(), outLow.data(), input.size(), 48000, 0.0f, 0.1f);
    lb.process(input.data(), outHigh.data(), input.size(), 48000, 0.0f, 1.0f);

    float rmsLow = computeRms(outLow.data(), outLow.size());
    float rmsHigh = computeRms(outHigh.data(), outHigh.size());
    fprintf(stderr, "    RMS at subLevel=0.1: %.6f, RMS at subLevel=1.0: %.6f\n",
            rmsLow, rmsHigh);
    ASSERT_TRUE(rmsHigh > rmsLow);
}

TEST(low_band_produces_energy_below_input_freq) {
    auto input = makeSine(200.0f, 0.5f, 48000, 0.2f);
    std::vector<float> output(input.size());
    ...
    float energyBelow100 = 0.0f;
    size_t bin100 = static_cast<size_t>(100.0f * fftSize / 48000.0f);
    for (size_t k = 1; k < bin100 && k < fftSize / 2; ++k) {
        energyBelow100 += mag[k];
    }
    fprintf(stderr, "    Energy below 100 Hz: %.6e\n", energyBelow100);
    ASSERT_TRUE(energyBelow100 > 1e-8f);
}

int main() {
    int passed = 0, failed = 0;
    fprintf(stderr, "\n=== Exciter: Low-Band Tests ===\n\n");
