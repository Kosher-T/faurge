...
print(f"Using {len(profiles)} speaker profiles (88D each)")

# ── Step 3: Clustering ──
print("\n═══ Step 3: Clustering ═══")
labels, centroids, thresholds, speaker_ids, X, X_scaled, sil_scores, inertias = \
    run_clustering(profiles, N_CLUSTERS)

# ── Step 3b: Cluster evaluation plots ──
print("\n═══ Step 3b: Cluster evaluation plots ═══")
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

ks = sorted(sil_scores.keys())
sil_vals = [sil_scores[k] for k in ks]
inert_vals = [inertias[k] for k in ks]

ax1.plot(ks, sil_vals, "o-", color="#2196F3", linewidth=2, markersize=8)
ax1.axvline(x=N_CLUSTERS, color="red", linestyle="--", alpha=0.7, label=f"Selected k={N_CLUSTERS}")
...
ax2.plot(ks, inert_vals, "s-", color="#FF9800", linewidth=2, markersize=8)
ax2.axvline(x=N_CLUSTERS, color="red", linestyle="--", alpha=0.7, label=f"Selected k={N_CLUSTERS}")
...
plt.tight_layout()
eval_path = CLUSTER_DATA_DIR / "cluster_evaluation.png"
...
ax.set_title("t-SNE of 88D Speaker Profiles (colored by cluster)")
...
# ── Step 6: Validation ──
validate_results(labels, floors, centroids, thresholds)

# ── Step 7: Save outputs ──
print("\n═══ Step 7: Saving outputs ═══")
save_outputs(profiles, labels, centroids, thresholds, floors, speaker_ids, CLUSTER_DATA_DIR)
