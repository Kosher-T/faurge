...
def compute_f0_features(audio: np.ndarray, sr: int = SR) -> np.ndarray:
    """
    Extract F0 (pitch) statistics: mean, std, min, max, range, voiced_fraction.
    Uses librosa piptrack for pitch detection.
    Returns 6D vector.
    """
    pitches, magnitudes = librosa.piptrack(y=audio, sr=sr, fmin=50, fmax=500, n_fft=FFT_SIZE, hop_length=HOP_SIZE)
    ...
    voiced_mask = pitch_arr > 50

    f0_mean = np.mean(pitch_arr)
    f0_std = np.std(pitch_arr)
    f0_min = np.min(pitch_arr)
    f0_max = np.max(pitch_arr)
    f0_range = f0_max - f0_min
    voiced_frac = np.mean(voiced_mask) if len(voiced_mask) > 0 else 0.0

    return np.array([f0_mean, f0_std, f0_min, f0_max, f0_range, voiced_frac], dtype=np.float64)

# ─── Formant Features ────────────────────────────────────────────────

def compute_formant_features(audio: np.ndarray, sr: int = SR) -> np.ndarray:
    """
    Extract first 3 formant frequencies (F1, F2, F3) using LPC analysis.
    Returns 3D vector (Hz values).
    """
    ...
    pre_emph = 1.0
    audio_emph = lfilter([1, -pre_emph], [1], audio)

    n_coeffs = 12 + 2
    frame_len = len(audio_emph)
    windowed = audio_emph * hamming(frame_len)

    corr = np.correlate(windowed, windowed, mode="full")
    corr = corr[frame_len - 1: frame_len + n_coeffs]

    try:
        a = solve(toeplitz(corr[:-1]), -corr[1:])
    except Exception:
        return np.zeros(N_FORMANTS, dtype=np.float64)
    ...
    formants = np.zeros(N_FORMANTS, dtype=np.float64)
...
def compute_mfcc_features(audio: np.ndarray, sr: int = SR) -> np.ndarray:
    """
    Extract MFCC statistics: mean and std for first 13 MFCCs.
    Returns 26D vector.
    """
    mfccs = librosa.feature.mfcc(y=audio, sr=sr, n_mfcc=N_MFCC, n_fft=FFT_SIZE, hop_length=HOP_SIZE)
    mfcc_mean = np.mean(mfccs, axis=1)
    mfcc_std = np.std(mfccs, axis=1)
    return np.concatenate([mfcc_mean, mfcc_std]).astype(np.float64)
...
def run_clustering(
    ...
    Returns (labels, centroids_dict, thresholds_dict, speaker_ids, X, X_scaled, sil_scores, inertias).
    """
    ...
    if n_clusters not in k_range:
        km = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
        labels_k = km.fit_predict(X_scaled)
        silhouette_scores[n_clusters] = silhouette_score(X_scaled, labels_k)
    ...
    for c in range(n_clusters):
        ...
        centroids_dict[f"cluster_{c}"] = centroid
    ...
    return labels, centroids_dict, thresholds_dict, speaker_ids, X, X_scaled, silhouette_scores, inertias
...
def validate_results(
    ...
    centroid_vecs = np.array([centroids[f"cluster_{c}"] for c in range(n_clusters)])
    centroid_mses = []
