# ══════════════════════════════════════════════════════════════════════════════
# Step 1bc — Generate Dataset (with per-dim normalization)
# ══════════════════════════════════════════════════════════════════════════════
# Input: degraded metrics (80D, normalized)
# Output: clean metrics (80D, normalized)
...
for i, clip in enumerate(clean_clips):
    clean_metrics_list.append(extract_metrics_80d(clip))
    print(f"  Clip {i+1}/{len(clean_clips)}: done")

print(f"\nGenerating {DEGRADATIONS_PER_CLIP} degradations per clip...")
...
for i, clip in enumerate(clean_clips):
    ...
    for _ in range(DEGRADATIONS_PER_CLIP):
        degraded = degrade_with_gain(clip)
        degraded = np.clip(degraded, -1.0, 1.0)
        deg_m = extract_metrics_80d(degraded)
...
# Use clean metrics as the reference: they represent the "target" distribution.
