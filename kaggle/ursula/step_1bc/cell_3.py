# ══════════════════════════════════════════════════════════════════════════════
# Step 1bc — Metric Extraction (80D)
# ══════════════════════════════════════════════════════════════════════════════
# Tier 0 (67D): LTAS64 + LUFS + Crest + ZCR
# Tier 1 (13D): Spectral Centroid/Bandwidth/Flatness/Flux/Rolloff/Skewness/Kurtosis/Slope
#               + Band Energy Ratios (5 bands)
# Total: 80D per audio clip
...
def compute_stft(audio, sr=SR, fft_size=FFT_SIZE, hop_size=HOP_SIZE):
    """Return magnitude spectrum and frequency bins."""
    freqs, times, Zxx = scipy.signal.stft(audio, fs=sr, nperseg=fft_size, noverlap=fft_size - hop_size)
    mag = np.abs(Zxx)
    return freqs, mag

def compute_spectral_centroid(freqs, mag):
    """Center of mass of magnitude spectrum (Hz)."""
    power = mag ** 2
...
def compute_spectral_bandwidth(freqs, mag, centroid_hz):
    """Spread of energy around centroid (Hz)."""
    power = mag ** 2
    total_power = np.sum(power, axis=0) + 1e-10
...
def compute_spectral_flatness(mag):
    """Wiener entropy: geometric_mean / arithmetic_mean. 0=tonal, 1=noisy."""
    power = mag ** 2 + 1e-10
...
def compute_spectral_rolloff(freqs, mag, threshold=0.85):
    """Frequency below which threshold% of energy lies."""
    power = mag ** 2
...
def compute_spectral_skewness(freqs, mag):
    """Third moment of spectral distribution."""
    power = mag ** 2
    total_power = np.sum(power, axis=0) + 1e-10
    centroid = np.sum(freqs[:, None] * power, axis=0) / total_power
...
def compute_spectral_kurtosis(freqs, mag):
    """Fourth moment — peakiness of spectrum."""
    power = mag ** 2
    total_power = np.sum(power, axis=0) + 1e-10
    centroid = np.sum(freqs[:, None] * power, axis=0) / total_power
...
def compute_spectral_slope(freqs, mag):
    """Linear regression on log-magnitude spectrum (dB/octave tilt)."""
...
def compute_band_energy_ratios(freqs, mag, sr=SR):
    """Energy in 5 perceptual bands normalized to total."""
...
print("Metric extraction: 80D (Tier 0: 67D + Tier 1: 13D)")
print("  Tier 0: LTAS64 + LUFS + Crest + ZCR")
print("  Tier 1: Centroid, Bandwidth, Flatness, Flux, Rolloff, Skewness, Kurtosis, Slope, BandRatios(5)")
test_m = extract_metrics_80d(clean_clips[0])
