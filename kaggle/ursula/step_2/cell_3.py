...
def extract_metrics_81d(audio, sr=SR):
    """Extract 81D metric vector: 80D (Tier 0+1) + RMS Energy."""
    # Tier 0
    ...
    return np.concatenate([ltas, lufs, crest, zcr, tier1, rms]).astype(np.float32)

print("Metric extraction: 81D (Tier 0: 67D + Tier 1: 13D + RMS: 1D)")
test_m = extract_metrics_81d(clean_clips[0])
